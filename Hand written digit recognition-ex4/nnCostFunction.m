...
J = (1/m)*sum( sum( -p.*log(a3) - (1-p).*log(1-a3)));

reg  = (lambda/(2*m))* sum(sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));

J = J + reg;

%delta_3 = a3 - y;
%delta_2 = delta3*Theta2 .*a2;
...
for t=1:m,
    a1t = X(t,:);
    a2t = sigmoid(Theta1*a1t');
    a2t = [ones(size(a2t',1),1) a2t'];
    a3t = sigmoid(a2t*Theta2');
    par3 = a3t - p(t,:);
    e=par3*Theta2;
    par2 = e(:,2:end).*sigmoidGradient((Theta1*a1t'))';    
     c   =(par2'*a1t);
     d   =(par3'*a2t);     
    Delta1 = Delta1 + c;
    Delta2 = Delta2 + d;
